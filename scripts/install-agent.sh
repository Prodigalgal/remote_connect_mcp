...
if [ -z "$MAX_TOTAL_CHILD_PROCESSES" ]; then
  ...
  if [ "$MAX_TOTAL_CHILD_PROCESSES" -lt 32 ]; then MAX_TOTAL_CHILD_PROCESSES=32; fi
  if [ "$MAX_TOTAL_CHILD_PROCESSES" -gt 256 ]; then MAX_TOTAL_CHILD_PROCESSES=256; fi
fi
case "$MAX_TOTAL_CHILD_PROCESSES" in
  ''|*[!0-9]*) echo "REMOTE_CONNECT_MCP_AGENT_MAX_TOTAL_CHILD_PROCESSES must be an integer" >&2; exit 1 ;;
esac
if [ "$MAX_TOTAL_CHILD_PROCESSES" -lt 1 ] || [ "$MAX_TOTAL_CHILD_PROCESSES" -gt 4096 ]; then
  echo "REMOTE_CONNECT_MCP_AGENT_MAX_TOTAL_CHILD_PROCESSES must be between 1 and 4096" >&2
  exit 1
fi
