...
function App() {
  ...
  // Each page starts at its own top.  Otherwise a long task/audit page can
  // leave the next page opened halfway down, especially on mobile.
  useEffect(() => {
          ...
          {Array.from(new Set(nav.map((item) => item.group))).map((group) => (
            <div className="nav-group" key={group}>
              ...
              {nav.filter((item) => item.group === group).map((item) => (
                <button className={item.id === page ? 'nav-item active' : 'nav-item'} key={item.id} onClick={() => navigate(item.id)} aria-current={item.id === page ? 'page' : undefined} title={item.label}>
...
function AccessControl({ token, machines, projects }: { token: string; machines: Machine[]; projects: Project[] }) {
  ...
  const reload = useCallback(async () => {
    ...
    setBusy(true); setMessage('')
    try {
      const [tokenPage, machineRows, projectRows, sessionRows] = await Promise.all([listMcpTokens(token), listMachineGrants(token, principal), listProjectMembers(token, principal), listExecutionSessions(token, principal)])
      setTokens(tokenPage.items); setMachineGrants(machineRows); setProjectMembers(projectRows); setSessions(sessionRows)
      if (principal.trim()) setQuota(await getQuota(token, principal.trim()))
      else setQuota(null)
    } catch (error) { setMessage(error instanceof Error ? error.message : '读取访问控制数据失败') }
    finally { setBusy(false) }
  }, [token, principal])
  useEffect(() => { void reload() }, [reload])
  const issue = async () => {
    setBusy(true); setMessage(''); setIssued('')
    try { const result = await issueMcpToken(token, { principal_id: principal || undefined, display_name: displayName || undefined, expires_in_seconds: Number(expires) || 0, scopes: ['mcp:read', 'mcp:execute', 'mcp:project'] }); setPrincipal(result.principalId); setIssued(result.token); setMessage('Token 只显示一次，请立即复制；Center 仅保存摘要。'); await reload() }
    catch (error) { setMessage(error instanceof Error ? error.message : '生成 MCP Token 失败') }
    finally { setBusy(false) }
  }
  const copyIssued = async () => { if (issued) { await navigator.clipboard.writeText(issued); setMessage('Token 已复制') } }
  const grantMachineAccess = async () => { if (!principal.trim() || !machineId) return; setBusy(true); try { await grantMachine(token, { principal_id: principal.trim(), machine_id: machineId, scopes: ['read', 'execute'] }); await reload() } catch (error) { setMessage(error instanceof Error ? error.message : '授予机器访问失败') } finally { setBusy(false) } }
  const grantProjectAccess = async () => { if (!principal.trim() || !projectId) return; setBusy(true); try { await grantProject(token, { principal_id: principal.trim(), project_id: projectId, scopes: ['read', 'write'] }); await reload() } catch (error) { setMessage(error instanceof Error ? error.message : '授予项目访问失败') } finally { setBusy(false) } }
  return <>
    <PageIntro kicker="SECURITY" title="访问控制" action="刷新" onAction={() => void reload()} />
    <section className="split-grid">
      <div className="panel form-panel"><span className="section-kicker">MCP PRINCIPAL</span><h3>签发用户 Token</h3><p>Token 是连接器的唯一主体身份；明文只在签发响应中出现一次。</p><label>Principal ID（可空）<input value={principal} onChange={(event) => setPrincipal(event.target.value)} placeholder="例如 user-alice" /></label><label>显示名称<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Alice" /></label><label>有效期<select value={expires} onChange={(event) => setExpires(event.target.value)}><option value="3600">1 小时</option><option value="86400">1 天</option><option value="2592000">30 天</option><option value="0">不过期（仅受撤销控制）</option></select></label><button className="primary full" onClick={() => void issue()} disabled={!token || busy}>签发 Token</button>{issued && <div className="issued-token"><code>{issued}</code><div><button className="secondary" onClick={() => void copyIssued()}>复制明文</button></div></div>}</div>
      <div className="panel form-panel"><span className="section-kicker">GRANTS</span><h3>授予资源范围</h3><p>用户 Token 默认没有机器/项目权限；在这里显式授予，避免多用户结果串话。</p><label>目标机器<select value={machineId} onChange={(event) => setMachineId(event.target.value)}><option value="">选择机器</option>{machines.map((machine) => <option key={machine.id} value={machine.id}>{machine.name} · {machine.id}</option>)}</select></label><button className="secondary full" onClick={() => void grantMachineAccess()} disabled={!principal.trim() || !machineId || busy}>授予机器 read + execute</button><label>目标项目<select value={projectId} onChange={(event) => setProjectId(event.target.value)}><option value="">选择项目</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name} · {project.id}</option>)}</select></label><button className="secondary full" onClick={() => void grantProjectAccess()} disabled={!principal.trim() || !projectId || busy}>授予项目 read + write</button></div>
    </section>
    {message && <p className="form-message" role="status">{message}</p>}
    {quota && <section className="panel"><div className="section-heading"><div><span className="eyebrow">QUOTA</span><h3>{quota.principalId}</h3></div></div><div className="upgrade-meta"><span>活动任务 <b>{quota.activeTasks}/{quota.maxActiveTasks}</b></span><span>排队任务 <b>{quota.queuedTasks}/{quota.maxQueuedTasks}</b></span><span>活动会话 <b>{quota.activeSessions}/{quota.maxSessions}</b></span><span>已预约传输 <b>{quota.reservedTransferBytes.toLocaleString()}/{quota.maxTransferBytes.toLocaleString()}</b></span><span>已传输 <b>{quota.transferredTransferBytes.toLocaleString()}</b></span></div></section>}
    <section className="panel table-panel"><div className="section-heading"><div><span className="eyebrow">TOKENS</span><h3>已签发 Token</h3></div></div>{tokens.length ? tokens.map((item) => <div className="table-row" key={item.tokenId}><span><strong>{item.displayName}</strong><small>{item.principalId} · {item.tokenId}</small></span><span>{item.scopes.join(', ')}</span><span>{item.revokedAt ? 'revoked' : item.expiresAt ? new Date(item.expiresAt).toLocaleString() : '不过期'}</span><button className="secondary compact-action" onClick={() => void revokeMcpToken(token, item.tokenId).then(reload)}>撤销</button></div>) : <div className="filtered-empty">暂无 Token</div>}</section>
    <section className="panel table-panel"><div className="section-heading"><div><span className="eyebrow">SESSIONS</span><h3>执行会话与车道</h3></div></div>{sessions.length ? sessions.map((item) => <div className="table-row" key={`${item.principalId}:${item.sessionId}`}><span><strong>{item.sessionId}</strong><small>{item.conversationId} · {item.machineId}</small></span><span>{item.status} · {item.capability ?? '—'}</span><span>{item.workspacePolicy ?? 'shared_serial'} · {item.laneMode ?? 'write'}</span><button className="secondary compact-action" onClick={() => void closeExecutionSession(token, item.principalId, item.sessionId).then(reload)}>关闭</button></div>) : <div className="filtered-empty">暂无会话</div>}</section>
    <section className="panel table-panel"><div className="section-heading"><div><span className="eyebrow">RESOURCE ACL</span><h3>机器与项目授权</h3></div></div>{machineGrants.map((item) => <div className="table-row" key={`m:${item.principalId}:${item.machineId}`}><span>机器 · {item.machineId}</span><span>{item.principalId}</span><span>{item.scopes.join(', ')}</span><button className="secondary compact-action" onClick={() => void revokeMachine(token, { principal_id: item.principalId, machine_id: item.machineId }).then(reload)}>撤销</button></div>)}{projectMembers.map((item) => <div className="table-row" key={`p:${item.principalId}:${item.projectId}`}><span>项目 · {item.projectId}</span><span>{item.principalId}</span><span>{item.scopes.join(', ')}</span><button className="secondary compact-action" onClick={() => void revokeProject(token, { principal_id: item.principalId, project_id: item.projectId }).then(reload)}>撤销</button></div>)}{!machineGrants.length && !projectMembers.length && <div className="filtered-empty">暂无授权</div>}</section>
  </>
}
