...
[![CI](https://github.com/Prodigalgal/remote_connect_mcp/actions/workflows/java-react.yml/badge.svg)](https://github.com/Prodigalgal/remote_connect_mcp/actions/workflows/java-react.yml)
...
目标架构和演进边界见 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)，异步调用契约见 [`docs/ASYNC_CONTRACT.md`](docs/ASYNC_CONTRACT.md)，详细语言、运行时、原生构建和前端选型见 [`docs/TECH_STACK.md`](docs/TECH_STACK.md)，Java/React 发布门禁见 [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md)，组件化 CI/CD 见 [`docs/COMPONENT_RELEASES.md`](docs/COMPONENT_RELEASES.md)，当前实现/生产阻塞见 [`docs/STATUS.md`](docs/STATUS.md)，SLO/告警见 [`docs/OBSERVABILITY.md`](docs/OBSERVABILITY.md)，传输评估见 [`docs/TRANSPORT.md`](docs/TRANSPORT.md)，HTTP 工件网关接口见 [`docs/ARTIFACT_GATEWAY.md`](docs/ARTIFACT_GATEWAY.md)，command/desktop/browser 组件级升级合同见 [`docs/COMPONENT_UPGRADES.md`](docs/COMPONENT_UPGRADES.md)。Java 25 Center/Agent 与 React 控制台是唯一生产实现。一个物理终端默认只有一个向 Center 注册的 `command-agent` 身份；桌面能力由同安装包启动的用户会话 `desktop-companion` 提供，浏览器能力由有界的本机 Browser Worker 提供，不增加额外 machine ID 或 Token。确需隔离时才为同一终端显式注册多个 Agent。
...
Java/React 已完成 Center/Console 生产切换：`java/` 提供 Java 25 多模块 Center、command-agent、desktop-companion 和 browser-agent 三个 Native 构建目标，异步 MCP、事务任务/输出/工件适配和本机能力桥接；`web/` 提供独立 React/Vite 控制台并可用 Admin Token 读取 Center API。Java Center 的持久化路线固定为 PostgreSQL + Liquibase，不使用 Flyway。仓库不再包含 Go 运行时、旧部署或旧安装路径。
...
域名在当前生产部署中固定，不随组件升级改变：

| 类型 | 命名约定 | 用途 |
| --- | --- | --- |
| 生产入口 | `remote-connect-mcp-center.*`、`remote-connect-mcp-agent.*`、`remote-connect-mcp-console.*` | 分别承载 MCP/Center、Agent 通道和控制台；组件版本升级不改变 URL |
...
1. `command` 把命令写入 Center，默认不等待 Agent，立即返回持久化 `task_id`。
...
6. ChatGPT 使用非阻塞的 `task_read` 获取有界输出，不需要让一次 HTTP 调用等待整个命令完成。
...
`command` 支持 `idempotency_key`。同一机器使用相同键和相同参数重试时，Center 返回原任务而不会重复执行；如果相同键对应不同参数，Center 会拒绝请求。ChatGPT 应为每个逻辑命令生成一次稳定键，并在连接超时重试时复用。
...
| 工具 | 用途 |
| --- | --- |
| `machines` | 使用 `operation=list/detail` 分页发现机器或按需读取单台机器详情 |
| `project` | 注册 Agent 本地项目，并异步创建/移除隔离 Git worktree；仓库内容不上传 Center |
| `command` | 在指定机器创建异步命令任务；服务端自动生成缺省幂等键 |
| `task_read` | 合并读取任务状态和有界输出，可选短暂等待状态变化 |
| `task_cancel` | 取消排队或正在运行的任务 |
| `desktop` | 仅对声明 `desktop` 能力的用户会话 Agent 提供有界截图、屏幕枚举、应用启动、点击、拖拽、按键、文本、剪贴板和窗口聚焦；截图以图片工件返回 |
| `browser` | 仅对声明 `browser` 能力且配置本机适配器的 Agent 投递结构化 Worker 请求；快照可返回最多 64 个 `rcm-ref-v1` 元素引用供后续动作复用；不上传 Cookie/CDP 凭据 |
| `artifact` | 使用 `operation=put/get/read` 做双向文件传输和按需读取引用；文件内容不进入 MCP 文本 |

当前唯一公开模型面固定为 8 个逻辑 Tool：`machines`、`command`、`desktop`、`browser`、`project`、`artifact`、`task_read`、`task_cancel`。旧 Tool 名称、旧字段和兼容开关不再注册。Browser Agent 不会把 Playwright/Patchright/Comoufox 的全部底层 API 一次性暴露；机器数量不会扩大 ChatGPT 的工具元数据。每次机器操作都必须显式传入 `machine_id`，范围统一放在 `scope` 对象中；`unrestricted` 必须明确填写，避免污染 ChatGPT Web 上下文。

MCP 返回专门的精简视图：`machines` 使用分页摘要，`task_read` 只返回有限输出页，任务状态不会回显提交时的环境变量或令牌；过长命令和错误文本会截断并标记。每个结果同时提供严格 `outputSchema` 和一份精简 JSON 文本，不在 MCP 文本内塞入大日志或文件字节；需要更多内容时使用 `next_cursor`/`offset` 或 Artifact 引用按需获取。
...
1. GitHub Actions 在每次 `main` 推送后构建 Linux amd64/arm64 与 Windows amd64 的 Native Image，并发布一个不可变的 `java-v0.0.0-main.<run>` 预发布版本；正式 `java-vX.Y.Z` 标签发布同样经过完整门禁。每个 Agent 资产都提供 `.sha256`；发布只接受完整 Native bundle，Windows ARM64 未纳入发布矩阵；
...
| `RCM_CENTER_RELEASE_TAG_PREFIX` | `java-` | 发布 URL 的 Tag 前缀；本仓库 Java 发布使用 `java-v1.2.3`，控制台版本仍填写 `v1.2.3`；使用普通 `v*` Tag 时设为空 |
| `RCM_CENTER_RELEASES_API_URL` | `https://api.github.com/repos/Prodigalgal/remote_connect_mcp/releases` | 控制台版本目录的 GitHub Releases API；只读取公开元数据，不保存 GitHub 凭据 |
...
| `RCM_CENTER_ARTIFACT_STORE` | `filesystem` | 工件字节存储后端；默认使用 Center 持久卷上的 `filesystem`，需要时才切换为 `http` 内部对象网关。外部对象存储不是硬性依赖；生产不得使用测试内存实现 |
...
| `RCM_CENTER_ARTIFACT_HTTP_TIMEOUT_SECONDS` | `30` | 可选 HTTP 对象网关单次请求超时，范围 1–120 秒 |
| `RCM_CENTER_ARTIFACT_RETENTION_SECONDS` | `604800` | 默认工件 TTL（7 天）；只影响未 pinned 的已完成工件，签名 URL TTL 独立配置 |
| `RCM_CENTER_ARTIFACT_WEB_RETENTION_SECONDS` | 同默认 TTL | Web→Agent 文件 TTL；范围 5 分钟至 365 天 |
| `RCM_CENTER_ARTIFACT_LARGE_RETENTION_SECONDS` | 同默认 TTL | 大于等于 1 GiB 文件 TTL；范围 1 小时至 365 天 |
| `RCM_CENTER_ARTIFACT_TEXT_RETENTION_SECONDS` | 同默认 TTL | 文本/JSON/XML/日志文件 TTL；范围 5 分钟至 365 天 |
| `RCM_CENTER_ARTIFACT_URL_TTL_SECONDS` | `900` | 浏览器/Viewer 签名 URL TTL（15 分钟）；与文件 TTL 无关 |
...
| `REMOTE_CONNECT_MCP_AGENT_POLL_INTERVAL_MS` | `5000` | 断线重连的指数退避基线；范围 250–60000 ms，不参与正常事件驱动长轮询 |
| `REMOTE_CONNECT_MCP_AGENT_LONG_POLL_SECONDS` | `25` | Agent 单次 HTTPS 长轮询等待秒数（1–25）；事件/取消/配置到达即返回，禁止关闭事件驱动路径 |
...
| `REMOTE_CONNECT_MCP_AGENT_BROWSER_BINARY` | 同目录或 `browser/rcm-browser-agent` | 独立 Browser Agent Native 二进制；未配置时仅按正式 bundle 布局查找，缺失则拒绝 browser 任务，不直接执行适配器命令 |
...
`command` 中的 `cwd` 可以留空或使用相对路径，例如 `src`、`backend`。绝对路径、`..` 穿越、符号链接或 Windows Junction 解析到工作区外时会被 Agent 拒绝；Center 也会按目标平台做一次词法校验。切换机器的目录策略需要更新 Agent 配置并重新注册（删除身份文件后使用新的一次性注册 Token）。
...
Windows amd64 使用 Native Image Agent，并由内置 Windows Task Scheduler 以 SYSTEM 身份在系统启动时运行。Native Image 是控制台程序，不能直接注册成 SCM ServiceMain；启动任务还配置失败重启，因此不会出现 SCM 7000/7009 超时。Windows ARM64 当前不在正式发布矩阵中；请在管理员 PowerShell 7 中执行：
...
Java Center 暴露 `/mcp` Streamable HTTP 端点，使用无会话 MCP 连接协议；没有旧 Gateway 根路径映射或 Go 协议入口。

MCP 服务端不能强制绕过 ChatGPT Web 自己的动作确认；是否显示“始终允许”由 ChatGPT Business 工作区策略决定。

## 构建与发布

Java 25 使用仓库内 Gradle Wrapper；React 控制台使用 pnpm。所有测试、JVM 包、React 资源和 Native Image 都由 GitHub Actions 执行，开发机和目标宿主机不执行任何编译或打包，避免编译峰值占满内存。CI 会输出 SHA-256、manifest、SBOM/证明和可安装归档；Windows/Linux Agent ZIP 可直接交给对应安装器，验证器会拒绝路径穿越、重复文件和超大归档。Native Image 构建机缺少 `native-image` 时不得把 JVM JAR 冒充原生发布物。
...
资产。手动触发只做构建验证，不创建 Release。推送前无需在本机安装 JDK、Gradle 或 GraalVM；
...
- Enrollment Token：由 Center Admin API 按机器名生成的一次性、1 小时至 30 天令牌，只供新 Agent 首次注册使用；Center 不接受共享环境变量注册凭据。
