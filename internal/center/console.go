...
<body>
  ...
  <div id="app" class="hidden">
    ...
    <main>
      ...
      <section class="panel"><h2>新增机器注册令牌</h2><div class="body stack">
        ...
        <div class="muted">Token 与机器名称绑定、默认仅可使用一次，Center 只保存 SHA-256 摘要。机器暂时离线不影响生成；请在有效期内完成首次启动。</div><div id="enrollmentNotice" class="muted"></div><div id="enrollmentError" class="error"></div>
        <div class="table-wrap"><table><thead><tr><th>机器</th><th>状态</th><th>使用</th><th>过期时间</th><th>操作</th></tr></thead><tbody id="enrollmentTokens"></tbody></table></div>
      </div></section>
  ...
  <script>
    ...
    async function createEnrollment(){const name=$('enrollmentName').value.trim(),expires=Number($('enrollmentLifetime').value);$('enrollmentError').textContent='';$('enrollmentNotice').textContent='';if(!name){$('enrollmentError').textContent='请输入目标机器名称。';return}try{const result=await api('enrollment-tokens',{method:'POST',body:JSON.stringify({name,expires_seconds:expires,max_uses:1})});latestEnrollment={name,token:result.token};$('enrollmentToken').value=result.token;renderEnrollmentConfig();$('enrollmentNotice').textContent='已生成一次性 Token。明文不会再次显示，请立即复制到目标机器。';await refresh()}catch(error){$('enrollmentError').textContent=error.message}}
