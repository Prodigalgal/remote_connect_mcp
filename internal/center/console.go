...
<body>
  ...
  <script>
    const $=id=>document.getElementById(id);let token=sessionStorage.getItem('remoteConnectMcpAdminToken')||'';let machines=[],tasks=[],upgrades=[],accessTokens=[],selected='';
    ...
    async function refresh(){try{const [machineData,taskData,upgradeData,tokenData]=await Promise.all([api('machines'),api('tasks?limit=100'),api('upgrades?limit=20'),api('tokens')]);machines=machineData.machines||[];tasks=taskData.tasks||[];upgrades=upgradeData.upgrades||[];accessTokens=tokenData.tokens||[];render();$('refreshState').textContent='已刷新 '+new Date().toLocaleTimeString()}catch(error){$('refreshState').textContent='刷新失败：'+error.message}}
    ...
    $('loginButton').onclick=login;$('token').onkeydown=e=>{if(e.key==='Enter')login()};$('run').onclick=run;$('cancel').onclick=cancel;$('reloadOutput').onclick=loadOutput;$('startUpgrade').onclick=startUpgrade;$('generateToken').onclick=generateToken;$('copyToken').onclick=copyToken;$('rotateToken').onclick=rotateAccessToken;$('logout').onclick=()=>{sessionStorage.removeItem('remoteConnectMcpAdminToken');location.reload()};
