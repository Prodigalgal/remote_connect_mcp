...
<body>
  ...
  <div id="app" class="hidden">
    ...
    <main>
      ...
      <section class="panel"><h2>新增机器注册令牌</h2><div class="body stack">
        <div class="row"><input id="enrollmentName" placeholder="机器名称，例如 oraclecloud-hs-new"><select id="enrollmentPlatform"><option value="linux">Linux</option><option value="windows">Windows</option></select><select id="enrollmentLifetime"><option value="0" selected>长期 · 单机专用</option><option value="3600">一次性 · 1 小时</option><option value="86400">一次性 · 24 小时</option><option value="604800">一次性 · 7 天</option><option value="2592000">一次性 · 30 天</option></select><button class="primary" id="createEnrollment">生成机器 Token</button></div>
        <div class="row"><input id="enrollmentToken" type="password" readonly placeholder="生成后仅在这里显示一次"><button id="copyEnrollmentToken">复制 Token</button><button id="copyEnrollmentConfig">复制配置</button></div>
        <pre id="enrollmentConfig">先生成注册 Token。目标 Agent 注册成功后会获得每机独立身份，此 Token 随即失效。</pre>
        <div class="muted">长期 Token 与一个机器名称永久绑定，可用于该机器安装、重装和身份恢复，手工撤销前不会过期；一次性 Token 成功注册后失效。两种模式都只在 Center 保存 SHA-256 摘要。</div><div id="enrollmentNotice" class="muted"></div><div id="enrollmentError" class="error"></div>
        <div class="table-wrap"><table><thead><tr><th>机器</th><th>类型</th><th>状态</th><th>使用</th><th>过期时间</th><th>操作</th></tr></thead><tbody id="enrollmentTokens"></tbody></table></div>
      </div></section>
      <section class="panel"><h2>访问令牌轮换</h2><div class="body stack">
        <div class="row"><select id="tokenKind"><option value="mcp">MCP · ChatGPT</option><option value="admin">Admin · 控制台</option><option value="enrollment">Enrollment · Agent 注册</option></select><input id="tokenGrace" type="number" min="0" max="86400" value="300" title="旧令牌宽限秒数"><button id="generateToken">生成</button></div>
        <div class="row"><input id="newToken" type="password" autocomplete="new-password" placeholder="新令牌，至少 32 个字符"><input id="confirmToken" type="password" autocomplete="new-password" placeholder="再次输入新令牌"><button id="copyToken">复制</button><button class="danger" id="rotateToken">确认轮换</button></div>
        <div class="muted">Center 只持久化令牌 SHA-256 摘要。K8S Secret/env 发生变化时会作为恢复操作接管对应令牌；正常重启不会覆盖控制台轮换结果。</div><div id="tokenNotice" class="muted"></div><div id="tokenError" class="error"></div>
        <div class="table-wrap"><table><thead><tr><th>类型</th><th>来源</th><th>更新时间</th><th>旧令牌有效至</th></tr></thead><tbody id="accessTokens"></tbody></table></div>
      </div></section>
