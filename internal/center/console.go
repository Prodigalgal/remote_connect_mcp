...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}
    ...
    .stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.stat,.panel{background:var(--panel);border:1px solid var(--line);border-radius:10px}.stat{padding:16px}.stat b{font-size:25px;display:block}.stat span,.muted{color:var(--muted)}
...
<body>
  ...
  <div id="app" class="hidden">
    ...
    <main>
      <section class="stats"><div class="stat"><b id="machineCount">0</b><span>已注册机器</span></div><div class="stat"><b id="onlineCount">0</b><span>在线机器</span></div><div class="stat"><b id="runningCount">0</b><span>活动任务</span></div></section>
  ...
  <script>
    const $=id=>document.getElementById(id);let token=sessionStorage.getItem('remoteConnectMcpAdminToken')||'';let machines=[],tasks=[],selected='';
    ...
    async function refresh(){try{const [machineData,taskData]=await Promise.all([api('machines'),api('tasks?limit=100')]);machines=machineData.machines||[];tasks=taskData.tasks||[];render();$('refreshState').textContent='已刷新 '+new Date().toLocaleTimeString()}catch(error){$('refreshState').textContent='刷新失败：'+error.message}}
    function render(){const names=Object.fromEntries(machines.map(m=>[m.id,m.name]));$('machineCount').textContent=machines.length;$('onlineCount').textContent=machines.filter(m=>m.online).length;$('runningCount').textContent=tasks.filter(t=>['queued','dispatching','running','cancel_requested'].includes(t.status)).length;
    ...
    $('loginButton').onclick=login;$('token').onkeydown=e=>{if(e.key==='Enter')login()};$('run').onclick=run;$('cancel').onclick=cancel;$('reloadOutput').onclick=loadOutput;$('logout').onclick=()=>{sessionStorage.removeItem('remoteConnectMcpAdminToken');location.reload()};
