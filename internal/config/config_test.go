...
func TestTokenMustBeSet(t *testing.T) {
	...
	stateDir := t.TempDir()
	t.Setenv("REMOTE_MCP_STATE_DIR", stateDir)
	t.Setenv("REMOTE_MCP_TOKEN", "")
	t.Setenv("REMOTE_MCP_PORT", "18765")
	if err := os.WriteFile(filepath.Join(stateDir, "token"), []byte("stale-generated-token"), 0o600); err != nil {
		t.Fatal(err)
	}
	_, err := Load()
	if err == nil || !strings.Contains(err.Error(), "REMOTE_MCP_TOKEN is required") {
...
func TestManualToken(t *testing.T) {
	...
	t.Setenv("REMOTE_MCP_TOKEN", "manually-set-token")
	config, err := Load()
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloudflareAutoProvisionConfig(t *testing.T) {
	t.Setenv("REMOTE_MCP_WORKSPACE", t.TempDir())
	t.Setenv("REMOTE_MCP_STATE_DIR", t.TempDir())
	t.Setenv("REMOTE_MCP_TOKEN", "mcp-token")
	t.Setenv("REMOTE_MCP_CF_AUTOPROVISION", "1")
	t.Setenv("CLOUDFLARE_API_TOKEN", "cloudflare-token")
	t.Setenv("CLOUDFLARE_ACCOUNT_ID", "")
	t.Setenv("CLOUDFLARE_ZONE_ID", "")
	t.Setenv("CLOUDFLARE_ZONE_NAME", "")
	t.Setenv("REMOTE_MCP_PUBLIC_HOSTNAME", "mcp.example.com")
	t.Setenv("REMOTE_MCP_TUNNEL_NAME", "")
	t.Setenv("REMOTE_MCP_TUNNEL_CONFIG", "")
	t.Setenv("REMOTE_MCP_QUICK_TUNNEL", "1")
