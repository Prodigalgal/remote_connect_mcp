...
: "${REMOTE_MCP_ENV_FILE:=$ROOT/remote-mcp.env}"
export REMOTE_MCP_ENV_FILE
...
BIN="$ROOT/dist/linux-$ARCH/remote-mcp"
if [ ! -x "$BIN" ]; then
  ...
  CGO_ENABLED=0 GOOS=linux GOARCH="$ARCH" go build -mod=mod -trimpath \
    -ldflags "-s -w -X main.version=0.1.0" -o "$BIN" ./cmd/remote-mcp
fi
