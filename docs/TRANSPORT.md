...
Java 25 的标准 `HttpClient` 没有通用 HTTP/3 客户端 API。引入 Netty incubator、外部 `curl` 或其他 QUIC provider 会增加 Native Image 反射配置和旁路库；在没有真实丢包/长 RTT 基准之前，不能把实验传输变成唯一链路。
...
- Agent 请求带有有界的 `X-RCM-Transport-Capabilities`/`X-RCM-Transport-Preferred`
  ...
  `/agent/v1/*` 只选择 HTTPS，保证当前 provider 选择稳定；
...
- HTTP/3 不可用、失败或性能不佳时，必须选择 HTTPS/WebSocket，不得让 Agent 离线。
