...
状态：设计完成；第一阶段主体、任务归属、幂等隔离、执行车道以及显式执行会话/结果通道、机器/项目 ACL 已实施，会话生命周期及 Desktop/Browser 会话隔离待补

本文把“多个用户、多个 Web 对话、多个 MCP 连接和多台终端”拆成独立关系。它是
`P2-05-lite` 的设计基线。当前第一阶段已经把不透明 MCP Token 映射到 Principal，
并将主体、连接元数据、幂等键、执行车道、显式执行会话、任务专属结果通道和机器/项目 ACL 接入任务生命周期；会话生命周期、
...
以下是目标逻辑表。当前轻量实现已通过 `015`–`017` 将主体、车道、执行会话标识和任务专属结果通道落到
`rcm_task`；独立的会话/连接/ACL/lease 表仍按后续子任务拆分实施：
