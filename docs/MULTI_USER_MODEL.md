...
状态：设计完成；第一阶段主体、任务归属、幂等隔离、执行车道以及显式执行会话/结果通道、机器/项目 ACL、会话合同持久化已实施，会话自动过期、能力 ACL 及 Desktop/Browser 会话隔离待补

本文把“多个用户、多个 Web 对话、多个 MCP 连接和多台终端”拆成独立关系。它是
`P2-05-lite` 的设计基线。当前第一阶段已经把不透明 MCP Token 映射到 Principal，
并将主体、连接元数据、幂等键、执行车道、显式执行会话、任务专属结果通道、机器/项目 ACL 和会话最新合同接入任务生命周期；会话自动过期、
Desktop lease 和 Browser Context 隔离仍需后续切片完成。未迁移的旧连接器仍可使用
...
以下是目标逻辑表。当前轻量实现已通过 `015`–`019` 将主体、车道、执行会话标识、任务专属结果通道、
机器/项目 ACL 和会话最新合同落到 PostgreSQL；独立的连接/lease 表仍按后续子任务拆分实施：
