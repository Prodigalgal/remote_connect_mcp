...
- command、browser 以及无用户会话时的 desktop 直启共享 `REMOTE_CONNECT_MCP_AGENT_MAX_TOTAL_CHILD_PROCESSES` Agent 级进程总预算（默认随并发增长但封顶 256，范围 1–4096）。监督器在任务启动时先为根进程保留名额，观察到新增后代时动态占用；预算耗尽立即终止该任务并回传原因，任务退出或桌面进程 `onExit()` 时释放名额。Agent 关闭、重启或升级时，直启预算会原子标记为关闭、停止所有仍登记的 GUI 进程并释放本地/全局名额，避免新旧 Agent 交叠产生孤儿进程。用户会话中的 desktop companion 仍由自身的 IPC/启动上限管理，避免把第二个物理进程的资源生命周期错误地绑定到 command-agent。
