...
Java 25 Center/Agent 与 React 控制台已经完成既有 v0.1.28 生产发布；当前代码新增 Artifact Transport v2：`artifact_put`/`artifact_get`、流式对象存储、Agent 原子收发、短期签名文件对象 URL，以及 Agent→Center `Content-Range`/偏移确认和 Web→Agent HTTP Range 断线续传。它仍需要最新 GitHub Actions 编译/集成测试、数据库迁移和目标环境回归后才能宣称生产可用。既有生产数据库、Agent 升级和旧 Go 回滚事实保持不变；ChatGPT Web 真实附件渲染、跨重启故障矩阵和特殊文件边界仍是后续门禁。
...
| MCP | Streamable HTTP `/mcp`、Bearer 校验、精简工具面、分页结果 | v0.1.15 公网验收已作为兼容基线保留；当前 v0.1.28 继续使用同一 `/mcp` 契约 |
...
| Desktop | 独立 `desktop` Native 目标的用户会话 companion；截图/区域截图、屏幕枚举、启动、单击/双击/右击、移动指针、拖拽、组合按键、剪贴板、窗口聚焦和文本输入通过受保护 loopback IPC；伴侣退出时会终止其登记的 GUI 子进程；没有用户会话时 command-agent 只返回明确的 companion 不可用错误，不包含桌面直启回退；Linux AWT/Java2D/X11 JNI 元数据只进入 Desktop Native 包 | 协议校验、`DesktopCompanionClientTest`、`DesktopCompanionServerTest`、GitHub Actions preview 与 `local-cmcc-debian` GUI canary；Windows 多会话/UAC、Wayland 和跨桌面回归仍待补齐 |
| Browser | 独立 `browser` Native 目标的单任务 supervisor；command-agent 负责 Center 生命周期、超时、日志和工件，browser-agent 再启动本机 Playwright/Patchright/Comoufox Worker；参考 Worker 增加 hover/check/select/wait-for-selector/历史导航/文本读取等常用动作，仍保持快照、引用和输出有界 | `BrowserTaskRunnerTest`、`BrowserTaskRunner`、Node 静态检查、Windows/Linux GUI canary `navigate`；snapshot/截图/下载、持久会话和跨引擎回归仍待补齐 |
...
| Artifact Transport v2 | 新增 `file_transfer` 协议记录、`020` Liquibase 元数据表、filesystem/HTTP 流式 ObjectStore、Agent GET/PUT、真实路径/哈希校验、8 MiB `Content-Range` 分块、Center partial spool、Web→Agent HTTP Range 和精简 `artifact_put`/`artifact_get`/`artifact_read` MCP 工具；文件内容不进入任务 JSON 或 MCP 文本 | 本机仅完成静态检查；GitHub Actions 需验证 Java/Native/JDBC 迁移、重试、重复 chunk 和对象流；ChatGPT Web 附件渲染、跨重启故障矩阵与生产回归尚待实现 |
...
1. Native Image：v0.1.28 正式 tag Release 已完成；Linux amd64/arm64、Windows amd64 原生构建、原生迁移烟测、SPDX SBOM、Sigstore keyless 签名、OIDC Artifact Attestation 和 Agent 资源门禁均由 GitHub Actions 完成。Windows/Linux 发布物为包含旁路运行库的平铺 ZIP，旧裸可执行文件保留回退；Windows ARM64 暂保留 JVM/Go 兼容路径。开发机不执行编译。
2. PostgreSQL：生产库已使用独立 `remote_connect_mcp_prod` schema/database 完成 Liquibase 与旧 Go 状态导入，当前保留 9 台 Agent、1403 个任务/输出、0 个工件；v0.1.28 生产迁移 Job 成功。工件已经抽象为 `ArtifactStore`，新写入可走独立持久卷文件对象或有界 HTTPS 对象网关，数据库只保存 key、大小、MIME 和 SHA-256；旧 `artifact_data` 仍保留用于一次性懒迁移。仍需补齐对象保留/GC、卷备份恢复、高并发抢占、Center 重启场景、长输出压测和正式恢复演练。
...
4. Desktop Agent：基础截图、屏幕枚举、输入、剪贴板和 Windows 窗口聚焦已具备；command-agent 会原子发布伴侣 scope 策略，伴侣在 IPC 前复核真实路径和合同过期时间；unrestricted Agent 收到更窄的 per-task path/workspace 合同时，伴侣不再因缺少机器级根目录而误拒绝，而是以合同根作为最小边界；伴侣在重启时回收其登记的 GUI 子进程；策略文件缺失或损坏时生产加载路径默认收窄到 Agent 状态目录，不再隐式放大为整机权限；Linux arm64 Plasma/X11 canary 已通过 screens/截图和 Center 路由，Windows 多会话/UAC、Wayland、多显示器输入和跨桌面回归仍需专门验收。
...
8. 控制台：基础管理流程、项目/worktree、全局搜索/基础筛选和有界任务输出查看可用；机器、项目、任务、升级和审计列表已按服务端 `has_more` 增量加载，升级页可只重排队单个失败目标；审计页提供管理员确认后清理一年以前记录的有界入口，MCP 创建的任务与控制台创建的任务在审计来源中分别标记为 `mcp`/`console`，任务编排已覆盖 command/desktop/browser 与显式范围；MCP 机器/项目列表使用摘要投影，JSON 文本有 192 KiB 最后防线，图片只内联不超过 512 KiB，较大工件改用 SHA-256/Console 引用；实时推送、审计详情和无障碍/视觉回归门禁尚未达到生产级完整度。
