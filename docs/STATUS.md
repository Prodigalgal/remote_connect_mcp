...
5. 构建拆分：`command-agent`、`desktop-companion`、`browser-agent` 已分别建立 Gradle Native 目标、Docker artifact 目标和 Release ZIP/校验流程；GitHub Actions `34795775084` 已通过 Linux amd64/arm64、Windows amd64 的四目标编译与烟测，目标机安装/升级回归仍待执行。
...
- 当前代码已将 Native 构建拆为 `command-agent`、`desktop-companion`、`browser-agent` 三个目标；命令 Agent
  ...
  `MAX_BROWSER_WORKERS` 限制。GitHub Actions `34795775084` 已通过新的拆分构建、Native 烟测和运行时回收门禁；生产验收仍需目标机安装、升级和真实桌面/浏览器回归。
