...
| Desktop | 独立 `desktop` Native 目标的用户会话 companion；截图/屏幕枚举、启动、点击/拖拽、组合按键、剪贴板、窗口聚焦和文本输入通过受保护 loopback IPC；伴侣退出时会终止其登记的 GUI 子进程；没有用户会话时的 command-agent 直启进程也登记在有界预算中，并在 Agent 正常退出、升级或 JVM shutdown hook 中统一回收，避免重启留下无界孤儿进程 | `DesktopCompanionClientTest`、`DesktopProcessBudgetTest`、协议测试，以及 GitHub Actions Native smoke 通过 |
