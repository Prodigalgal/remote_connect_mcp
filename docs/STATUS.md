...
1. Native Image：Windows amd64 的 Center/Agent 原生链路此前曾在本机构建并通过烟测；当前策略是不在开发机编译，GitHub Release 工作流已加入 OIDC Artifact Attestation，并改为在匹配架构的 `ubuntu-24.04`/`ubuntu-24.04-arm` runner 上构建和执行 Linux amd64/arm64 烟测，再由 Windows runner 构建 Windows amd64。云端工作流、正式 Release 和签名/SBOM 仍待实际执行。Windows/Linux Agent 发布物均为包含旁路运行库的平铺 ZIP，旧裸可执行文件保留回退；Windows ARM64 暂保留 JVM/Go 兼容路径。
