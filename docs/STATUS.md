...
Java 25 Center/Agent 与 React 控制台已经完成既有 v0.1.28 生产发布；当前开发批次已补齐 Artifact Transport v2：`artifact_put`/`artifact_get`、流式对象存储、Agent 原子收发、短期签名文件对象 URL、Content-Range/HTTP Range 断线续传、状态机/ACK、幂等预约、资源配额和安全边界，并完成会话/车道/桌面/浏览器隔离代码。接下来需要一次统一 GitHub Actions 编译/集成测试，然后进行数据库迁移、目标环境和 ChatGPT Web 真实附件回归；在这些门禁完成前不宣称本批次生产可用。
