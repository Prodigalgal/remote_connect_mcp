...
生产应用 `remote-connect-mcp-java-production` 在 Kubernetes 中为 `Synced/Healthy`；Java Center、Console、PostgreSQL 均为 Ready，Liquibase migration Job 为 Complete。生产镜像版本为 `v0.1.21`，而本轮代码提交为 `a3f2cd9`，两者尚未收敛。

| 能力 | 结果 | 证据与说明 |
| --- | --- | --- |
| Center 健康 | 通过 | `/api/v1/healthz` 返回 200，状态 `ok`，实现为 Java，版本 `v0.1.21` |
...
| MCP 授权会话 | 通过 | 有效 MCP Token 完成 `initialize`、会话建立和 `tools/list`；六个核心工具均存在 |
| MCP 机器列表 | 通过 | `machines_list` 调用成功，结果未发现 Token、密码、Secret 或私钥字段 |
| Admin 鉴权与基础分页 | 通过 | 有效 Admin Token 可读取机器列表；`items/offset/limit` 存在 |
| Admin 新分页投影 | 部分通过 | 生产 `v0.1.21` 尚未返回当前代码新增的 `total/has_more`，说明本轮分支尚未部署；不能把该项记为生产通过 |
...
| Agent 命令闭环 | 通过 | 一台在线 Oracle ARM64 Agent（身份已脱敏）执行固定 `printf`，状态 `completed`、Attempt 1、输出 29 字节；通过事件等待得到终态 |
...
| 升级在线/离线/失败回滚 | 待验收 | 生产登记列表包含离线 Agent；尚未在本批次启动升级活动或重排队失败目标 |
...
本轮提交 `a3f2cd9` 已在 GitHub Actions 完成大规模回归：

- 综合 CI `34925260218`：Go 测试/vet、四平台兼容构建、事件驱动检查和仓库卫生，成功；
- Java/React `34925262430`：PostgreSQL/Liquibase、JVM Ubuntu/Windows、React 构建、Linux amd64/arm64 Native 构建、MCP 冒烟和 Agent RSS 门禁，成功；
- 本机没有执行 Java、Gradle、Native Image、React 或正式安装包构建。

## 发现与后续闭环

1. 生产基础链路可以继续使用，但生产镜像仍是 `v0.1.21`；需通过 GitHub Actions 发布并经 GitOps 收敛到包含本轮代码的不可变版本，再复验 `total/has_more`、审计、对象网关和传输标头。
2. 生产命令闭环已通过一次低风险验证；第一次验证脚本因换行转义产生误报，修正后第二次按字节校验通过，不代表其他故障场景已经验收。
3. 下一批应在维护窗口内执行 Center/Agent 重启、断线恢复、长任务取消、资源超限、升级失败/离线领取和 WebSocket 反向代理演练。
